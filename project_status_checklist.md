...
- [x] Public/Student: image → OCR → "explain this document" plain-language flow, `POST /documents/explain-image`, gated to public+student. Public gets a standalone upload section on QuestionPage.jsx with the persistent disclaimer still visible; Student gets it injected as a normal turn into the existing multi-turn chat thread (including read-aloud support). Verified: correct plain-language explanation with disclaimer even on a non-legal test document, clean error message on unreadable images, student chat integration and lawyer/judge non-visibility both confirmed.
